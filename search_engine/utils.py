...
def build_query_vector(tokens, vocab, idf):
    ...
    for term in set(tokens):
        if term in vocab_index:
            ...
            tf_value = 1 + math.log(tf[term]) if tf[term] > 0 else 0
...
#     return dot / (norm1 * norm2) if norm1 and norm2 else 0.0
#
# def build_query_vector(weighted_tokens, vocab, idf):
#     vocab_index = {term: idx for idx, term in enumerate(vocab)}
#     tf = Counter()
...
#
#     query_vector = {}
#     for term in set(token for token, _ in weighted_tokens):  # Lặp qua các từ duy nhất
#         if term in vocab_index:
#             idx = vocab_index[term]
#             tf_value = 1 + math.log(tf[term]) if tf[term] > 0 else 0
#             tfidf = tf_value * idf[term]
